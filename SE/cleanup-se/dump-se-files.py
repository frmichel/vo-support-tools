...
optParser.add_option("--url", action="store", dest="url", default='',
                     help="The url of the SE to analyse.")

optParser.add_option("--output-file", action="store", dest="output_file", default='',
                     help="output file to write results. If no then output to stdout")
...
if options.url=='':
    optParser.error("Option --url must be affected a value")
    exit(1)
...
# Recursive method that go through the files tree of targeted url
# Take 2 parameters:
...
def ls_rec(url,f) :
    ...
    # Recursively call the function on each file present in the current directory
    for entry in entries:
        # check that filename doesn't begin by '/'
        # (workaround until gfal2.5 version is released)
        if entry[0] != '/':	
    ...
    # go through the enries map to print entries whose type is file	
    for (entry_key,entry_st) in entries_map.iteritems():
	# Check entry is a file
        if not stat.S_ISDIR(entry_st.st_mode):
	    # print the file line
	    if f!='':
	        f.write( mode_to_rights(entry_st.st_mode) + ' ' + str(datetime.datetime.fromtimestamp(int(entry_st.st_ctime)).strftime('%Y-%m-%d')) + ' ' + str(datetime.datetime.fromtimestamp(int(entry_st.st_mtime)).strftime('%Y-%m-%d')) + ' ' + str(entry_st.st_size) + ' ' + url+'/'+entry_key+'\n')
	    else:
	        print( mode_to_rights(entry_st.st_mode) + ' ' + str(datetime.datetime.fromtimestamp(int(entry_st.st_ctime)).strftime('%Y-%m-%d')) + ' ' + str(datetime.datetime.fromtimestamp(int(entry_st.st_mtime)).strftime('%Y-%m-%d')) + ' ' + str(entry_st.st_size) + ' ' + url+'/'+entry_key)

    # go through the entries map so that for each entry whose type is directory:
    # print it, and then call recursively the function on this directory
    for (entry_key,entry_st) in entries_map.iteritems():
...
# Close the file descriptor if a filename has been specified in argument
if outputToFile:
    f.close()
